 ...
 */
function decodeTempleData(encoded: string): TempleData | null {
  try {
 ...
 */
function extractShareData(shareUrl: string): string | null {
  try {
...
function isStraightLine(path: { x: number; y: number }[]): boolean {
  ...
  for (let i = 1; i < path.length - 1; i++) {
    ...
    const area = Math.abs(
      (last.x - first.x) * (point.y - first.y) -
 ...
 * Find path using BFS
 */
function findPath(grid: { [key: string]: Room }, start: { x: number; y: number }, end: { x: number; y: number }): { x: number; y: number }[] {
